...
async fn root() -> impl axum::response::IntoResponse {
    outer_template(maud::html! {
        h1."text-6xl mb-4" {"Hey! I am a sample app that's slow to respond."}
        h3."text-4xl mb-4" {"We have a few routes that repond differently."}

        h3."text-4xl mb-16"{"The root is the only route that responds without any waiting."}
...
fn outer_template(body: Markup) -> Markup{
    maud::html! {
        script src= "https://cdn.tailwind.com" {}
